...
public class DataProvider2 extends DataFileReader {

	public DataProvider2(String fileName) {
		super(fileName);
	}
			...
			for (String line = br.readLine(); line != null; line = br.readLine()) {
				...
				if(splitData.length>4){
					if(splitData[0].trim().equals(oringin.trim()) && splitData[2].trim().equals(destination.trim())){
						long departTime = Utils.parase2(splitData[1].trim());					
						long destTime = Utils.parase2(splitData[3].trim());
